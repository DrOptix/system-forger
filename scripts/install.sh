...
function main() {
        ...
        case "$1" in
            -u|--user)
                if [[ -n "$2" && "$2" != --* ]]; then
                    ...
                    echo "Error: '--user' requires an argument." >&2; exit 1
                fi
                ;;
            -t|--tags)
                if [[ -n "$2" && "$2" != --* ]]; then
                    ...
                    echo "Error: '--tags' requires an argument." >&2; exit 1
                fi
                ;;
            *)
    ...
    pushd "$SYSTEM_FORGER_REPO_ROOT" || {
        echo "Error: Failed to enter repository root '$SYSTEM_FORGER_REPO_ROOT'. Exiting." >&2
        exit 1
    }
    ...
    if [[ ! -f "$main_playbook_path" ]]; then
        echo "Error: Playbook '$main_playbook_path' not found in repository root." >&2
        popd; exit 1
    fi
    ...
    local ansible_cmd=(ansible-playbook "$main_playbook_path" \
     ...
     if [[ -n "$ansible_tags" ]]; then
         echo "Running with Ansible tags: '$ansible_tags'"
         ansible_cmd+=(--tags "$ansible_tags")
     else
         echo "Running entire playbook (no tags specified)."
     fi
    ...
    # the user is needed
    if [[ "$(id -u)" == "0" ]]; then
        echo "Script is running as 'root'. No 'become' password needed for Ansible."
    elif [[ -n "${ANSIBLE_BECOME_PASS+x}" ]]; then
        echo "Running as non-root. Using 'ANSIBLE_BECOME_PASS' from environment (non-interactive)."
        ansible_cmd+=(--become-pass "$ANSIBLE_BECOME_PASS")
    else
        echo "Running as non-root. 'ANSIBLE_BECOME_PASS' not set. Will prompt for become password (interactive)."
        ansible_cmd+=(--ask-become-pass)
    fi

    echo "Executing Ansible: ${ansible_cmd[*]}"
    "${ansible_cmd[@]}" || {
        echo "Error: Ansible playbook execution failed for '$playbook_name.yml'." >&2
        popd; exit 1
    }

    echo "Ansible playbook '$playbook_name.yml' completed successfully."
